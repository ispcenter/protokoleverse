...
task = pd.DataFrame(columns=["Тип Протокола", "№ протокола", "Модель", "Серийный номер"
	# , "Путь для поиска", "Дата"
	])
...
for i in range(len(task_0.index)):
	...
	task_0.loc[i][2].astype('int32') #"Серийный номер"
...
for i in range(len(task_0.index)):
	...
	task_0.loc[i][5].astype('int32'),	#"№ протокола"
	task_0.loc[i][0] if task_0.loc[i][4] != 'пс' else task_0.loc[i-j][0],	#"Модель"
	task_0.loc[i][6].astype('int32') #"Серийный номер"
	# 'path', #"Путь для поиска"
	# task_0.loc[i][7] #"Дата"
	]
...
task["№ протокола"].apply(str)

print(task)
print(task.loc[[40],["№ протокола"]])
print(type(task.loc[40]["№ протокола"]))
